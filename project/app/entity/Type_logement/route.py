...
@cross_origin(origin=["http://127.0.0.1:5274","http://195.15.228.250","*"],
              headers=['Content-Type','Authorization'],automatic_options=False)
@type_log.route('/type_log/ajouter', methods=['POST'])
def create():
    try:
        id=[doc.to_dict() for doc in db_type_log.stream()]#[-1]['id']
    ...
    if id:
        ...
        todo = db_type_log.document(id).get()
...
def read():
    all_todos = [doc.to_dict() for doc in db_type_log.stream()]
    return jsonify(all_todos), 200

@cross_origin(origin=["http://127.0.0.1","http://195.15.228.250","*"],headers=['Content-Type','Authorization'])
@type_log.route('/type_log/<int:ide>', methods=['GET'])
def read_ind(ide):
